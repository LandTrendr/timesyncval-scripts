...
def getTxt(file):
	...
	for line in txt:
		...
		if not line.startswith('#'): #skip commented out lines
			...
			elif title in ['ltcolumn', 'tscolumn']:
...
def binValidationData(csvData, ltCol, tsCol, ltbins, matches, plotCol="UID", 
yearCol="YEAR"):
	...
	dtypes = [(plotCol,'a32'), (yearCol,'f8'), ("TIMESYNC", 'a32'), ("LANDTRENDR", 'a32'), 
	...
	for ind,row in enumerate(csvData):
		...
		try:
			...
			binnedArray["LT_MATCH"][ind] = "BLANK"
	...
	for ind in nomatch_inds:
		...
		for alt in altYears:
			...
			try:
				...
				if outputData['UID'][ind+alt] != outputData['UID'][ind]: continue
...
# 	return newTS
# 	
def combineSet(disturbances):
	'''test if possible to combine TimeSync disturbance set into one event'''
	
	if ("STABLE" in disturbances) or ("RECOVERY" in disturbances):
		newTS = None
	elif all([disturbances[0] == i for i in disturbances]):
		newTS = disturbances[0]
	elif all([((i=="HARVEST") or (i=="SITE-PREPARATION FIRE")) for i in disturbances]):
		newTS = "Harvest_Site-preparation Fire".upper()
	else:
		newTS = None
...
def isolateEvents(tolerantData):
	...
	for gind,group in enumerate(consec_disturbances):
		...
		if len(group) == 1:
			...
			disturbances = []
			for row in group: disturbances.append(row[timesync_ind])
			
			#test if combinable using timesync column
			newTS = combineSet(disturbances)
			...
			if newTS: 
				...
				max_ind = [float(i[1]) for i in lt_mags].index(max([float(i[1]) for i in lt_mags]))
...
def main(parameter_file_path):
	...
	binnedData = binValidationData(inputData, parameters['ltcolumn'], parameters['tscolumn'], 
